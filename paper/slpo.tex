...
The target probability of everything else is simply
\( 1 - w_w \), and the predicted probability is
\[
\pi_\theta(y_{\overline{w}, 1} \mid x) = \sum_{v \neq y_{w,1}} \exp\left(\mathrm{logsoftmax}(z_{v, 1})\right).
\]
...
because the probabilities of all possible continuations sum to 1.}
...
\[
\pi_\theta(y_{w, 1}) \cdot \pi_\theta(y_{w, 2}) \cdot \pi_\theta(y_{\overline{w}, 3})
\]
\[
\pi_\theta(y_{w, 1}) \cdot \pi_\theta(y_{w, 2}) \cdot \pi_\theta(y_{w, 3}) \cdot \pi_\theta(y_{\overline{w}, 4})
\]
\[
\pi_\theta(y_{w, 1}) \cdot \pi_\theta(y_{w, 2}) \cdot \pi_\theta(y_{w, 3}) \cdot \pi_\theta(y_{w, 4}) \cdot \pi_\theta(y_{\overline{w}, 5})
\]
Summing across all possible positions results in 
\[
...
\cdot
\underbrace{
  \pi_\theta(y_{\overline{w},t} \mid x, y_{w,<t})
}_{2}
.
\]

Underbrace 1 represents the path down the tree of all possible sequences that 
follows the winning sequence, including a zero-length path.
Underbrace 2 represents the probability of the next token not being a winner.
As stated before, underbrace 2 is inclusive of all possible continuations
after position \(t\), since the probabilities of all continuations sum to 1.
