...
class MemorizationModel(torch.nn.Module):
  ...
  def forward(self, x):
    ...
    if x.size() != (self.batch_size,):
      raise ValueError(
        f"Invalid input shape: {x.size()}, expected {(self.batch_size,)}"
      )
  ...
  def logprob(self, example_idx, token_ids):
    if not 0 <= example_idx < self.batch_size:
      raise ValueError(
        f"Invalid example index: {example_idx}, num_examples={self.batch_size}"
      )

    if not 0 <= len(token_ids) <= self.seq_len:
      raise ValueError(
        f"Invalid token_ids length: {len(token_ids)}, seq_len={self.seq_len}"
      )
